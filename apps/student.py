...
from buttons.keyboardbuttons import btn_student_xonalari, admin_main_menu, btn_student_xona_turlari, \
    btn_students_xona, btn
from database.database import student_view_by_room_and_type, student_view_data_, student_delete_data, \
    student_insert_data
from states import state_admin, state_subadmin, state_educator
...
async def student_xona(m: m, state: s):
    ...
    """
    if m.text == "Chiqish":
        await m.answer("Asosiy menyu:", reply_markup=admin_main_menu)
        ...
        await state_admin.student_xona_turi.set()
...
async def student_xona_turi(m: m, state: s):
    ...
    if m.text == "Chiqish":
        ...
        await state_admin.student_students.set()
...
async def student_students(m: m, state: s):
    if m.text == "Chiqish":
        ...
        await state_admin.student_students_menu.set()
...
async def student_qoshish_name(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_surname.set()
...
async def student_qoshish_surname(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_qavat.set()
...
async def student_qoshish_qavat(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_room_number.set()
...
async def student_qoshish_room_number(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_room_type.set()
...
async def student_qoshish_room_type(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_gender.set()
...
async def student_qoshish_gender(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_photo.set()
...
async def student_qoshish_phone_number(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_aloqa.set()
...
async def student_qoshish_aloqa(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        await state_admin.student_qoshish_addres.set()
...
async def student_qoshish_addres(m:m, state:s):
    if m.text == "Bekor qilish":
        ...
        try:
            ...
            await m.answer("Talabani malumotlar bazasiga qo'shishda xatolik vujudga keldi iltimos qaytadan urinib ko'ring!")
